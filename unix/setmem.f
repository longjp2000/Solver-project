c$Id: setmem.f,v 1.2 2006/12/14 00:55:56 rlt Exp $
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      logical   pcomp
      integer   list,num,length,precis,ip,ipa, n,i
...
c         Call allocate for requested "length" expressed in bytes
          ...
          point = (length*ipa + mod(length*ipa,ipr))/ipr
          call fallocfn(np(n),point,ip,ipr)
...
c         Expand array if space available
          ...
          point = (length*ipa + mod(length*ipa,ipr))/ipr
          call freallocfn(np(n),point,ip,ipr)
