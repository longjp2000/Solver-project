c$Id: shl3di.f,v 1.1 2006/11/20 20:32:56 rlt Exp $
...
c         Modify rotational shape functions
	  ...
	  yy = yy + tr(2,3)*hh
          zz = zz + tr(3,3)*hh
          write(iow,2003) n,xx,sig,ma,yy,eps,zz
          if(ior.lt.0) then
            write(*,2003) n,xx,sig,ma,yy,eps,zz
          endif
        end do ! ll
      end do ! l
      return
...
c     Compute stress at point
      ...
      nhv   = nint(d(15))
      istrt = nint(d(84))
      call modlsd(d,ta,eps,hr(nh1+nn),hr(nh2+nn),nhv,istrt,dd,sig,isw)
