c$Id: shl3ds.f,v 1.1 2006/11/20 20:32:56 rlt Exp $
...
c     Form shape function terms for strains
      ...
      do i = 1,3
        j = mod(i,3) + 1
        k = mod(j,3) + 1
        shn(1,i) = (bd(i)*(el(i) - 1.0))*2.d0
        shn(2,i) = (cd(i)*(el(i) - 1.0))*2.d0
        shm1 = (bd(j)*el(k) + bd(k)*el(j))*2.d0
        shm2 = (cd(j)*el(k) + cd(k)*el(j))*2.d0
        a1(i) = aa(i)*shm1
        a2(i) = aa(i)*shm2
        b1(i) = bb(i)*shm1
        b2(i) = bb(i)*shm2
...
c     Compute membrane and bending strains
     ...
     &                  - (shp1(2,j,l) + shp2(1,j,l))*vl(6,j)
      ...
      norm(1) = norm(1) + (dd(1,i)*eps(i)
     &                  +  dd(1,2)*eps(2)
     &                  +  dd(1,4)*eps(3))*thk
      norm(2) = norm(1) + (dd(2,i)*eps(1)
     &                  +  dd(2,2)*eps(2)
     &                  +  dd(2,4)*eps(3))*thk
      norm(3) = norm(1) + (dd(4,i)*eps(1)
     &                  +  dd(4,2)*eps(2)
     &                  +  dd(4,4)*eps(3))*thk
...
c     Compute center (0,0) displacement
      ...
      if(nel.eq.3) then
        do i = 1,3
          x0(i) = (xl(i,1) + xl(i,2) + xl(i,3))/3.0d0
        end do ! i
      else
        do i = 1,3
