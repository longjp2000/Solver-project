c$Id: ylds2d.f,v 1.1 2006/11/20 20:32:56 rlt Exp $
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      real*8    toli,psi1,psi2,twoh3,d1,d2,e2,e3
      ...
      psi(1) = sqrt2*( eta(1) + eta(2))
      psi(2) = sqrt2*(-eta(1) + eta(2))
      psi(3) = eta(3)
...
c     Compute stresses on yield surface
      ...
      psi(2)  = psi(2)*f2
      psi(3)  = psi(3)*f2
      eta(1)  = sqrt2*(psi(1) - psi(2))
      eta(2)  = sqrt2*(psi(1) + psi(2))
      eta(3)  = psi(3)
