c$Id: fld2d4.f,v 1.1 2006/11/20 20:32:56 rlt Exp $
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      real*8   dvol(4),ee(6),sg(3,4),ace(2),body(3), detf, xsj
      real*8   rhol,rhoc, con1,con2
      real*8   df(3,3),f0(3,3),f1(3,3),bba(4,2,4)
...
c       Form acceleration at gauss point
        ...
        do i = 1,2
          ...
          ace(i) = ace(i)*con1 - body(i)*dvol(l)
        end do ! i
...
c       Store energy for material force computation
        ...
        elseif(isw.eq.16) then
          ...
          weng(l) = ss(1,l)*ee(1) + ss(2,l)*ee(2) + ss(3,l)*ee(3)
     &            + ss(4,l)*ee(4)
...
c     Compute Z-Z projections
      ...
      elseif(isw.eq.25) then
        ...
        call stcn2z(xl,ss,shp,dvol,lint,ndm,nel,9)
