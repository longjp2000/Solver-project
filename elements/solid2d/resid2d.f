c$Id: resid2d.f,v 1.1 2006/11/20 20:32:56 rlt Exp $
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      real*8    shp(3,*),sig(*),vc(3),ac(3),body(3)
...
c       Compute gravity, thermal, inertia, and stress contributions
        ...
        p(1,j) = p(1,j) + (body(1) - ac(1) - aj0*al(1,j))*shp(3,j)*xsj
     &                  -  aj1*sig(1) - aj2*sig(4) - aj3*sig(3)
        p(2,j) = p(2,j) + (body(2) - ac(2) - aj0*al(2,j))*shp(3,j)*xsj
     &                  -  aj1*sig(4) - aj2*sig(2)
...
c     Axisymmetry with torsion
      ...
      if(stype.eq.8) then
        ...
        aj0     = rr*lfac
        do j = 1,nel
          p(3,j) = p(3,j) + ((body(3) - (ac(3) + aj0*al(3,j))
