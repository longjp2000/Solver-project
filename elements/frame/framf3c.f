c$Id: framf3c.f,v 1.2 2006/12/14 00:54:17 rlt Exp $
...
c     Only SVK constitutive model. Other material models can be added in
c     modelsvq.
...
c        hatcj             : skew symmetric matrix associated with a vector
c        hmatcj            : matrix that appears in linearization of expmap
c        initializecj      : compute initial rotations
c        interpmatrix      : interpolation matrix for incremental variations
...
c        tmatcj            : computes matrix 't', that appears in linearization
...
c          Cayley transform(?)
c          Exponential map (-2).
c       We pick first one only for conserving solution, otherwise, second
...
c                 qmat  : interpolation matrices in linearization of rotation
...
c     Purpose: Sets variables in /bmcj1/,/bmcj2/ from element properties
...
c       Computation of position vectors and its derivative with respect to S at
...
c       Computation of spatial rotation vector from Lambda_n to Lambda_n+alpha
...
c                 centroids (Phi) and derivative with respect to S (Phi,s)
...
c                 elasticities by pushing forward their convective tensors
...
c       Compute rotation tensor at Gauss point by spherical interpolation
...
c                 qref    : rotation at undeformed configuration (quaterion)
