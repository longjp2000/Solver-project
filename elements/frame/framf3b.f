c$Id: framf3b.f,v 1.2 2006/12/14 00:54:17 rlt Exp $
...
c     Usage notes:
c         use rotational updates type 8 (exp map), included in this file.
...
c      - The paper by Simo and Vuquoc does has one serious inconsistency.
...
c        Lambda_n+1 and, among other things, material tangent not symmetric.
c        If theta means rather rotation vector from k-th iteration to k+1-th
c        iteration of rotation, then linearization is ok. This last approach
c        is one followed in current implementation
...
c     hatsvq             : skew symmetric matrix associated with a vector
...
c     tmatsvq            : computes matrix 'T' that appears in linearization
...
c     Input element parameters, done in inpt2d.f
...
c       3 for stress) in mass gauss points 4 doubles (for rotation)
        ...
        nh1 = (nh1 + 7)*gint + 4*mint  ! Change nh1 from history on section
...
c     Notes:   At each integration point total rotation is stored. That is,
c     rotation tensor that transforms inertial frame to section frame.
c     Observe that there is also posibility to store incremental rotation
c     tensor that only accounts for transformation between section frame
...
c     Select integration rule for stiffness (gint) and mass (mint) terms.
...
c     In stiffness: 4 (rotation quaternion)_n + 3 spatial curvature omega_n
...
c     Set distributed loading type. Use 'GFOL' or 'LFOL' in material definition
...
c                 epl(10)  : Total energy             (computed in pmacr2.f)
c                 epl(11)  : Angular momentum norm    (computed in pmacr2.f)

c     Notes:      for isw=13, independently of integrator, all variables
...
c       Recover data from history variables (rotation and curvature at tn+1)

        do kk = 1,4
          qrot1(kk)  = hr(nn2-1+kk)
        end do ! kk
        do kk = 1,3
          omega1(kk) = hr(nn2+3+kk)
        end do ! kk
...
c  Notes:      h = nn + sin(b)/b (eye-nn) + (1-cos(b))/n hatn,  b = norm(psi),
...
c     Notes:      The list of history variables initialized in following order:
...
c     REFE, NODE , x1, x2, x3 ... and coordinates of a node that defines t1, or
...
c     Rotation matrix is initialized for all stiffness GP of element
 ...
 200  nn1 = nh1
      ...
      do ip = 1,gint
        ...
        do k = 1,4
          hr(nn1-1+k) = rotqu0(k, ip)
          hr(nn2-1+k) = rotqu0(k, ip)
        end do ! k
...
c     Rotation matrix is initialized for all mass GP of element
      ...
      do ip = 1,mint
        ...
        do k = 1,4
          hr(nn1-1+k) = rotqu0(k, gint+ip)
          hr(nn2-1+k) = rotqu0(k, gint+ip)
        end do ! k
...
c       Computation of rotation_n+1, rotation_n+a at Gauss point (quaternion for
...
c           Rotation contribution
            ...
            tmp          = shp(2,a)*shp(2,b)*dx*ctan(3)

            do i = 1,3
              do j = 1,3
                s(l1+i+3,k1+j+3) = s(l1+i+3,k1+j+3) + 1.0d0
c     &                          + tmp2(i,j)*ctan(3)
c     &                          + hpin1(j,i)/(dt*ctan(3))
              end do ! j
            end do ! i
...
c                 shp   : (2,nel) derivative and shape functions at Gauss point

c     Output:     phi   : (3) position vector at Gauss point
c                phipr  : (3) derivative of phi w/r to reference arclength
c-----[--+----+----+----+----+----+----+----+----+----+----+----+----+-]
...
c                 xnode : nodal positions at tn+alpha (equilibrium point)
...
c-----[--+----+----+----+----+----+----+----+----+----+----+----+----+-]
      ...
      integer    nn1, nn2, nn3, nns, nh, nhs
...
c     flags, dynamics->true when running dynamics, cons->true when trans,cons
...
c       Recover data from history variables (rotation_n, rotation_ref)
        ...
        do k1 = 1,4
          qrot0(k1)  = hr(nn2-1+k1)
          qrotr(k1)  = hr(nn3-1+k1)
        end do ! k1

c       Recover convected rotational strain at t_n

        do k1 = 1,3
          omega0(k1) = hr(nn2+3+k1)
        end do ! k1
...
c       Computation of position vectors and its derivative with respect to S

        call positionsvq(xnode , nel, shp, phi , phipr)

c       Compute spatial rotation vector that takes Lambda_n to Lambda_n+1
c       Also its derivative, for computation of curvature
...
c       Computation of rotation tensor (matrices) at time t=0, tn and tn+1
...
c       Compute convected stresses and material tangent and push to spatial ones

        call modelsvq(d, hr(nh1+nns), hr(nh2+nns), nh,
     &                strain, stress, cc, isw)
        call pushfsvq(rotmat, stress, cc, sstress, scc)

c       Compute contribution from distributed forces (conservative or follower)
c       and contribution to tangent
...
c       Store data to history variables (rotation_n+1 and rot.strain_n+1)
...
c     Notes:      Only valid for quadratic potentials, using subroutine model
c-----[--+----+----+----+----+----+----+----+----+----+----+----+----+-]
...
c     Purpose: Update rotation matrix given spatial incremental rotation
c              vector,
