c$Id: framf3e.f,v 1.1 2006/11/20 20:32:56 rlt Exp $
...
c    omega(3,gint) - Spatial curvatures at Gauss points for stiffness at t=n

c   Partition nh2

c    thts(3,gint)  - Spatial rotation vectors at Gauss points for stiffness
...
c    lobatt        - Flag that indicates if a Gauss-Legendre quadrature must
c                    be used for mass matrix (0) or a Gauss-Lobatto
c                    one (1)
...
c    rotqu0(4,13)  - Initial rotation quaternion at Gauss points for mass
c                    and stiffness and nodes
c-----[--.----+----.----+----.-----------------------------------------]
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      integer   i, k1,k2,k3,k4,k5,k6,nt1,nt2,nh
...
c       Pointers for variables in partition of Common h used by this routine
         ...
         nt2 = mct0
         nt1 = nh1 + nt2    ! First stress history term at t_n
         nt2 = nh2 + nt2    ! First stress history term at t_n+1
...
c     Computation of tangent stiffness and residual vector
      ...
      elseif(isw.eq.3 .or. isw.eq.6) then
        ...
        call init3f(d)
        call tsrf3f(d,ul,xl,s,p,ndf,ndm,nst,hr(k1),hr(k3),hr(k4),hr(k5),
...
c     Output of variables
      ...
      elseif(isw.eq.4 .or.isw.eq.8) then
        ...
        call init3f(d)
        call outp3f(d,ul,xl,s,p,ndf,ndm,nst,hr(k1),hr(k3),hr(k4),hr(k5),
     &              hr(nh1+mct0),hr(nh2+mct0),nh,isw)
...
c     Update of database just before start of a new time step
      ...
      elseif(isw.eq.12) then
        ...
        call init3f(d)
        call tsdb3f(hr(k1),hr(k2),hr(k3),hr(k4),hr(k5),hr(k6))
...
c     and total energies
      ...
      elseif(isw.eq.13) then
        ...
        if(theta(1).ne.0.d0.and.ttim.ne.0.d0) then
          call init3f(d)
          call agen3f(d,ul,xl,ndf,ndm,hr(k1),hr(k3),hr(k4),hr(k5),
     &                hr(nh1+mct0),hr(nh2+mct0),nh,isw)
        endif
...
c     Initialize data base values
      ...
      elseif(isw.eq.14) then
        ...
        call indb3f(hr(k1),hr(k2),hr(k3),hr(k4),hr(k5),hr(k6))
...
c-----[--.----+----.----+----.-----------------------------------------]
c - ARGUMENTS
c i o   xlbds(4,gint)  : Quaternions associated with rotation matrices at
c                        Gauss points for stiffness at t=n
c i o   omega(3,gint)  : Curvatures at Gauss points for stiffness at t=n
c i o   thts(3,gint)   : Current spatial rotation vectors at Gauss points for
c                        stiffness between t=n and t=n+alpha
c i o   thtpm(3,gint)  : Current derivative with respect to S of spatial
c                        rotation vectors at Gauss points for stiffness between
c                        t=n and t=n+alpha
c-----[--.----+----.----+----.-----------------------------------------]
c CALLED BY: FRM3FD
...
c-----[--.----+----.----+----.-----------------------------------------]
c - ARGUMENTS
c i     xlbds(4,gint)  : Quaternions associated with rotation matrices at
c                        Gauss points for stiffness at t=n
...
c     Recovering of current total translational displacements at t=n+alpha
...
c       Computation of material strains and internal stresses at t=n+alpha
...
c     Transfer of spatial velocities at nodes at t=n+1 and computation of
c     velocities at t=n
...
c   o   fiprim(3)      : Derivative of fi with respect to S at that point
c-----[--.----+----.----+----.-----------------------------------------]
...
c-----[--.----+----.----+----.-----------------------------------------]
c - ARGUMENTS
c i     fiprim(3)      : Derivative of fi with respect to S at that point
...
c - ARGUMENTS
c i     exf(3)         : Current external forces vector in global coordinates
...
c     Computes part of tangent stiffness matrix corresponding to follower
c     forces along line of centroids (external preassure)

c-----[--.----+----.----+----.-----------------------------------------]
c - ARGUMENTS
c i     eforce(3)      : External forces vector in current coordinates
c i     centr(3,3)     : Current orthonormal frame along which load is defined
...
c-----[--.----+----.----+----.-----------------------------------------]
c - ARGUMENTS
c   o   xlbds(4,gint)  : Quaternions associated with rotation matrices at
c                        Gauss points for stiffness at t=n
c   o   xlbdd(4,mint)  : Quaternions associated with rotation matrices at
c                        Gauss points for mass at t=n
c   o   omega(3,gint)  : Curvatures at Gauss points for stiffness at t=n
c   o   thts(3,gint)   : Current spatial rotation vectors at Gauss
c                        points for stiffness between t=n and t=n+alpha
c   o   thtpm(3,gint)  : Current derivative with respect to S of spatial
...
c     Update spatial curvature given a certain incremental rotation vector and
c     its derivative with respect to S
c-----[--.----+----.----+----.-----------------------------------------]
c - ARGUMENTS
...
c-----[--.----+----.----+----.-----------------------------------------]
c - ARGUMENTS
c i o   xlbda(4,gint)  : Quaternions associated with rotation matrices at
c                        Gauss points for stiffness at t=n
c i o   xlbdd(4,mint)  : Quaternions associated with rotation matrices at
c                        Gauss points for mass at t=n
c i o   omega(3,gint)  : Curvatures at Gauss points for stiffness at t=n
c i o   thts(3,gint)   : Current spatial rotation vectors at Gauss
c                        points for stiffness between t=n and t=n+alpha
c i o   thtpm(3,gint)  : Current derivative of spatial rotation vectors
c                        at Gauss points for stiffness between t=n and t=n+alpha
...
c     Computes angular momentum, and kinetic, potential and total energies
c     at start of a new time step and prints then in corresponding output
c     files
c-----[--.----+----.----+----.-----------------------------------------]
c - ARGUMENTS
c i     xlbds(4,gint)  : Quaternions associated with rotation matrices at
...
c i     thts(3,gint)   : Rotation vectors at Gauss points for stiffness between
c                        t=n and t=n+alpha
c i     thtpm(3,gint)  : Derivative with respect to S of rotation vectors
c                        at Gauss points for stiffness between t=n and t=n+alpha
c-----[--.----+----.----+----.-----------------------------------------]
c CALLED BY: FRM3FD
...
c       Computation of the velocities at t=n+1 and current configuration and
c       its derivative with respect to S
...
c       Computation of derivative with respect to S of current configuration
c       and current rotation matrix
...
c     Obtains variables of commons /bm3f1/,/bm3f2/ from element properties
c-----[--.----+----.----+----.-----------------------------------------]
c CALLED BY : FRM3FD
c CALL :      INIR3F
c             PZERO, VECP, PMOVE
...
c     Initialize rotation matrices for Gauss points of stiffness & mass & nodes
c-----[--.----+----.----+----.-----------------------------------------]
c CALLED BY : INIT3F
c CALL :      SHP1D
...
c i     xlbds(4,gint)  : Quaternions associated with rotation matrices at
c                        Gauss points for stiffness at t=n
c i     omega(3,2*int) : Curvatures at Gauss points for stiffness at t=n
c i     thts(3,gint)   : Rotation vectors at Gauss points for stiffness between
c                        t=n and t=n+alpha
c i     thtpm(3,gint)  : Derivative with respect to S of rotation vectors at
c                        Gauss points for stiffness between t=n and t=n+alpha
c-----[--.----+----.----+----.-----------------------------------------]
c CALLED BY: FRM3FD
...
c     Computation and output of current configuration, strains and stresses
...
c       Computation of displacements and its derivative with respect to S
...
c     Reference polar
      ...
      if(debug) then
        call mprint(t1,1,3,1,'T1')
        call mprint(t2,1,3,1,'T2')
        call mprint(t3,1,3,1,'T3')
      endif
