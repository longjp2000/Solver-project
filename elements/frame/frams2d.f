c$Id: frams2d.f,v 1.1 2006/11/20 20:32:56 rlt Exp $
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      integer   i,ii,i1,istrt, j,jj,j1, ll,lint, mm,nn,nhh,nhv, n1,n2
      real*8    cs,sn,a1,a2,a3,a4,b1,b2,le,dx,dva,dvi,cfac,dfac,xjac
      real*8    energy, rhoa, rhoi, ctan1, ctan3
...
c       Increment history storage if necessary
      ...
      elseif(isw.eq.3 .or. isw.eq.6) then
        ...
        do ll = 1,lint
          ...
          dx = sg(2,ll)*xjac
          call b2mods(d,ul,forc,aa,energy,shp,hr(n1),hr(n2),
     &                ndf,nen,isw,ll)
