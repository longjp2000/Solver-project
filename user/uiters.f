c$Id: uiters.f,v 1.1 2006/11/20 20:33:27 rlt Exp $
...
c     Compute sparse storage for matrix
      ...
      if(isw.lt.0) then
        ...
        call elcnt(numel,nen,nen1,mr(np(31)),mr(np(33)),mr(np(151)),1)

c       Check for contact elements

        if(numcels.gt.0 .and. np(168).ne.0) then
          call elcnt(numcels,ncen,ncen1,mr(np(31)),mr(np(168)),
     &               mr(np(151)),-1)
        endif
        call sumcnt(mr(np(151)),numnp*ndf,kp)

        setvar = palloc(152,'USER2', kp, 1)
        call pelcon(numel,nen,nen1,mr(np(33)),mr(np(31)),mr(np(151)),
     &              mr(np(152)),kp,1)
        if(numcels.gt.0 .and. np(168).ne.0) then
