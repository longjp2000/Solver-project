c$Id: pmacr1.f,v 1.2 2006/12/08 00:12:35 rlt Exp $
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      logical    nomass,factor, cknon0, solvsv, unsfl, naninfck
      logical    dtfl(19)
      character  tname*5, fext*4, fnamr*132, cmtyp*4, lct(*)*15
      integer    iops,mops,i,j, elist, neqms, kk,k1,k2,k3,k4
      integer    nenp,gneq, toteq
      real*4     tary(2), etime , tt
      real*8     tops,ur,ee,etab,reln,step,propsv, ct(3,*)
      real*8     dot,mnorm, rnaugm, tdatabuf(3),rnorms(3)
...
c                               (lumped projection)
c     [stre,gnod,k1,k2,k3]    - output global nod stresses k1 to k2 inc k3
c                               (lumped projection)
...
c         Consistent projection solution
     ...
     &                     numnp,numnp,aengy,.false.)
            ...
            enddo ! i
            call pltstr(hr(np(58)),hr(np(57)+numnp),hr(fp(6)),
     &                  numnp,ndm,fa)
...
c       Output sets
        ...
        call prtstr(hr(np(43)),hr(np(57)+numnp),hr(fp(6)),
...
c       Output residual norm
        ...
        if(f8o) then
          if(pfeap_on) then
            ...
            rnorms(2) = rnorm1
            rnorms(3) = rnormn
            call pfeapsr(rnorms,tdatabuf, 3)
            rnorm  = rnorms(1)
            rnorm1 = rnorms(2)
            rnormn = rnorms(3)
            toteq  = numteq
...
c         check initital iterate solution for possible line search)
          ...
          if(abs(aengy-ee).le.tol*rnmax .or. linear
     &       .or. rnorm/dble(toteq) .lt. rnorm1/rnormn*sqrt(tol)*1.d-3
     &       .or. abs(aengy-ee).lt.enzer ) then
            if(lv.gt.1) then
...
c           Set pointers then factor and solve equations
          ...
          call pexpd(hr(np(42)+nneq),hr(np(26)),mr(np(31)),ndf,nneq)
...
c         Solve rigid body equations
          ...
          if(rbody) then
            call pformrx(mr(np(31)),mr(np(100)),ndf,numnp,neqms)
...
c     List outputs
      ...
      if(pcomp(lct(l),'list',4)) then
        k1 = abs(ct(1,l))
        k1 = max(1,min(3,k1))
        call prtrel(hr(np(26)),ttim,ndf,niols(k1),iolist(1,k1),prth)
...
c     Compute/output work: Energy = U x R
...
c     Save reactions to file: fsav.rea & fsav.re#  (# = 0 to 99)
...
61      fnamr = fsav
        ...
        if(exst) then
          k1 = k1 + 1
          if(k1.le.9) then
            write(fext(3:3),'(i1)') k1
          elseif(k1.le.99) then
            write(fext(2:3),'(i2)') k1
          else
            write(ilg,3005)
            write(iow,3005)
...
c       N.B.  Always save current state to file 'fsav.rea'
        ...
        do kk = 1,2
          ...
          do i = 1,numnp
            etab = 0.0d0
            do k2 = 1,ndf
              etab = max(etab,abs(hr(fp(5)+k2)))
            end do ! k2
            if(etab.gt.ur) then
              write(ios,*) i,(hr(point),point=fp(5)+1,fp(5)+ndf)
            endif
            fp(5) = fp(5) + ndf
          end do ! i
          write(ios,*) '  '
...
c                       each time step. (default value is 1.0 and normally
c                       should be used.) 'gap' is maximum gap permitted for
c                       convergence.
...
c     Establish list of elements with slaved nodes
      ...
      call pelbld(mr(np(33)),mr(np(31)),mr(np(111)),
...
c     [ntan]gent,mate,#mate - Numerically compute element tangents for mate#
c     [ntan]gent,elem,#elmt - Numerically compute element tangent for elmt
...
c       Interchange 'pmass' with all processors
        ...
        if(pfeap_on) then
          setvar = palloc(111,'TEMP1',mf*kk,2)
          call pfeapsr(hr(np(126)),hr(np(111)),mf*kk)
          setvar = palloc(111,'TEMP1',0,2)
        endif
...
3001  format(' *ERROR* PMACR1: Compute nodal stresses before error')
...
3005  format(' *ERROR* PMACR1: Too many reaction files: Delete files')
