c$Id: ploadi.f,v 1.1 2006/11/20 20:32:37 rlt Exp $
...
c                     Data stored in blank common for later use
c-----[--.----+----.----+----.-----------------------------------------]
      implicit  none
...
c     Allocate memory
      ...
      elseif(isw.eq.2 .and. numsl.gt.0) then
        ...
        close(ios)
        setvar = palloc( 37, 'SLODI', nei, 1 )
        setvar = palloc(241, 'SLODR', ner, 2 )
...
210       do i = 1,nsv,8
          ...
          if(nint(td(1)).ne.0) then
            ...
            call pstore(ns,nv,ixl,valu,mr(np(37)+nei),hr(np(241)+ner))
            nei = nei + ns
            ner = ner + nv
            go to 2
          endif
