c$Id: rinput.f,v 1.1 2006/11/20 20:32:37 rlt Exp $
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      character titl*4,dnam*15, fext*4, type*4
...
c       Allocate inertia / R arrays
        ...
        setvar = palloc( 98,'RINER', nrbody*9     ,   2)  ! Intertia of body
        setvar = palloc( 99,'RIRB ', nrbody*nrbdof,   1)  ! Equation numbers
        setvar = palloc(104,'RLAMB', nrbody*54    ,   2)  ! Orientation
        setvar = palloc(107,'RMASS', nrbody*1     ,   2)  ! Mass of body
        setvar = palloc(108,'RUROT', nrbody*6     ,   2)  ! Rotation params
        ...
        if(neqrb.eq.-5 .or.neqrb.eq.-6) then
          ...
          setvar = palloc(110,'REXIN', nrbody*36  ,   2)  ! Explicit Inertia
        endif
...
c       Allocate storage for joint data
        ...
        setvar = palloc(102,'RJNX ',9*iclink,  2)   ! Joint coords/angles/disp
...
c       Set final memory and basis vectors for rotational joints
        ...
        if(nrevs.gt.0) then
          setvar = palloc( 97,'REVO ',nrevs*9,   2)  ! Joint revolute data
          jt = 0
          jx = 0
          l1 = 0
          do i = 1,numjts
        ...
        setvar = palloc(103,'RJTU ',numjts*15,   2)  ! Joint solution params.
