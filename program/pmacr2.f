c$Id: pmacr2.f,v 1.1 2006/11/20 20:32:37 rlt Exp $
...
c     [tol,iter,itol,atol] - set residual and absolute convergence tolerances
...
c           CONVERGENCE: Need to reduce time step and do next solution
            ...
            aratfl = .false.
            if(int(ct(3,l)).ge.int(afact)) then
...
c           CONVERGENCE: Need to increase time step and do next solution
            ...
            elseif(int(ct(3,l)).le.iaopt) then
...
c           Terminate current loop
      ...
      elseif(autofli .and. lvautoi.eq.lv ) then
        ...
        if(ct(1,l).le.ct(1,n)) then
           ...
           if (rmeas .gt. 2.0d0) then   ! Iterations are too large backup
            write(iow,*) 'Old dt',dt
...
c     Augmented loop check: N.B. Do not check if augm and solution in same loop
...
c     Conventional loop checks
     ...
     &            .not.pcomp('    ',cjump,4)) then
        ...
        elseif(ct(1,l).le.ct(1,n)) then

          if(ljump .and. dt.eq.0.0d0) then  ! Test for zero time increment
            floop(2) = .false.
            lv       = lv - 1
            return
          endif
...
2013  format(/'   Solution tolerance = ',1p,1e12.5/)
