c$Id: ptimpl.f,v 1.1 2006/11/20 20:32:37 rlt Exp $
      subroutine ptimpl
...
c-----[--.----+----.----+----.-----------------------------------------]
     ...
     &       nsplts,ntplts,nmplts,nuplts,nvplts).gt.0 ) then
...
c         Set transient parameters for current
          ...
          if(max(nmplts,nsplts,nqplts,nrplts,ntplts,nuplts).gt.0) then
            if(.not.rfl) then
              if(ntplts.gt.0) then
                do i = 1,ntplts
                  tpl(i) = 0.0d0
...
c           call ploade(mr(np(31)),hr(np(30)),hr(np(40)),epl(9) )
