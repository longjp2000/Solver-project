c$Id: cdasol.f,v 1.1 2006/11/20 20:32:37 rlt Exp $
...
100   neq = max(1, neqs)
     ...
     1                  + dot(aui(jr+1),bi(j-jh),jh)
          bi(j) = bi(j) - dot(aur(jr+1),bi(j-jh),jh)
     1                  - dot(aui(jr+1),br(j-jh),jh)
        endif
      end do ! j
...
c     Unsymmetric part
     ...
     1                  + dot(ali(jr+1),bi(j-jh),jh)
          bi(j) = bi(j) - dot(alr(jr+1),bi(j-jh),jh)
     1                  - dot(ali(jr+1),br(j-jh),jh)
        endif
      end do ! j
...
c     Multiply by inverse of diagonal elements
      ...
      do j = is,neqt
        ...
        enr   = enr + abs(bdr*br(j)) + abs(bdi*bi(j))
      end do ! j
