c$Id: d4triu.f,v 1.1 2006/11/20 20:32:37 rlt Exp $
...
c     Reduce remaining symmetric equations in pairs
      ...
      if(ior.lt.0 .and. neq.ge.1000 .and. pfr) write(*,3000)

      do j = js, neq, 2

        if(ior.lt.0) then
          if((mod(j,nprti).eq.0.or.mod(j+1,nprti).eq.0) .and. pfr) then
            tt = etime(tary)
            write(*,3001) j,tary
          end if
        endif
...
2001  format(' *D4TRI WARNING* sign of diagonal changed when',
     & ' reducing',i10,' equations.'/' Step:',i7,' Iteration:',i6)
