c$Id: pmacio.f,v 1.1 2006/11/20 20:32:37 rlt Exp $
      subroutine pmacio (jct,lct,ct,wd,ed,nwd,nlp,nif,ll)
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      logical    pcomp,flg,errck,tinput,vinput,pflg
      integer    nwd,nlp,nnx,ll,llo,l,i,lc,lo,j,k,is
      integer    nif,nel,nei, ncmds, jct(*),ed(nwd)
      real*8     sv, ct(3,*)
      character  lct(*)*15,wd(nwd)*4,clab(2)*15,vv(3)*15,str*75
...
c     Input command
      ...
      if(pfeap_on) then
        call parmacio(clab,ct,ll,nn)
      else
        clab(1) = ' '
        do while( pcomp(clab(1),' ',1) )
          if(ior.lt.0) then
            write(str,2004) nn+1,ll-1
            call pprint(str)
          endif
...
c     End: Quit (q), Exit (e) or Stop (s)
     ...
     &     .or.pcomp(clab(1),'e   ',4)) then
          call pprint('   Exit with restart file save (y or n) :')
...
c         Store endif information in 'lzz'
...
3001  format(' *WARNING* Maximum number of macro statements =',i4
     &     ,/'    Use history edit to reduce or program will stop'
     &     ,' when',i4,' is reached')

3002  format(' *ERROR* PMACIO: Maximum number of macro instructions',
     &       '                  is limited to',i4)
