c$Id: prot02.f,v 1.1 2006/11/20 20:32:37 rlt Exp $
...
c               Dynamic updates for generalized alpha method (HHT and Newark)
c               ... following Simo & Vu- Quoc 1988

c      I. Romero April 2002 (modified from original Simo/Taylor urot03.f)
...
c     Several possible strategies for starting a new time step (predictor)
c     we select one of them here
...
c         Only one that must be used for now.

          if (updateopt.eq.0) then

            do i = 1,3
              dm(i) = 0.0d0
            end do ! i

c         Zero Acceleration (do not use)

          elseif (updateopt.eq.1) then
            do i = 1,3
              dm(i) = dt*(vn(i)+(0.5d0-beta)*an(i)*dt)
            end do ! i

c         Increment with Previous Relative Quaternion (do not use)

          elseif (updateopt.eq.2) then
            call pmove ( u1 , dm , 3 )
          endif
