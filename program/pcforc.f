c$Id: pcforc.f,v 1.1 2006/11/20 20:32:37 rlt Exp $
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      character type*4,name*5,fnam*132,fext*4,ftype*15
      logical   prt ,prth, errck,tinput,pcomp, lsave, trflg, clflg
      logical   oprt,oprth
      integer   ndm,ndf,numnp, iosave,i, n,nbc, ntyp(*)
      real*8    dotx,xmn, tmn,xl(3),f(ndf,numnp),x(ndm,numnp),td(16)
...
200   if(ior.lt.0) then
      ...
      ftype  = ' '
      errck = tinput(ftype,1,td,ndf+ndm)
      if(errck) go to 200
