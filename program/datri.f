c$Id: datri.f,v 1.1 2006/11/20 20:32:37 rlt Exp $
...
c     Use Sparse solver with minimum degree ordering (symmetric only)
      ...
      if(ittyp.eq.-2) then
        ...
        if(np(67).ne.0) then
          setval = palloc( 67, 'SPTAN' ,   0  ,  1) ! Delete present one
        endif
        setval = palloc( 67, 'SPTAN' , esp  ,  1) ! Add new one at end
        espo = esp
...
c       Symbolic triangular decomposition
     ...
     &            ad,ad,ad,mr(np(67)),mr(np(67)),esp,4,je)

c       Minimal dimension for working arrays

        if(esp.gt.espo) then
          setval = palloc( 67, 'SPTAN' , esp, 1)
        endif
...
c       Numerical triangular decomposition
        ...
        call sdrv(neqt,mr(np(47)),mr(np(48)),mr(np(93)+neqt),mr(np(94)),
     &            ad,ad,ad,mr(np(67)),mr(np(67)),esp,6,je)
