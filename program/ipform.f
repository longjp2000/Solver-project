c$Id: ipform.f,v 1.1 2006/11/20 20:32:37 rlt Exp $
...
c         Set local arrays and rotate to global frame
          ...
          call plocal(ld,id,ix(1,n1),ie(1,ma1),mr(fp(1)),
     &                xl1,ul1,ule1,tl1,p(nub),x,f,u,ud,t,un,dun,
     &                nrot1, rfl,rel, jsw)
          if(nrot1(1)+nrot1(2).gt.0) then
...
c           Set local arrays for element 2
            ...
            n     = n2
            fp(1) = ndf*nen*(ma1-1) + np(240)              ! iedof
            call plocal(ld(nst+1),id,ix(1,n2),ie(1,ma2),mr(fp(1)),
     &                  xl2,ul2,ule2,tl2,p(nst+nub),x,f,u,ud,t,un,dun,
     &                  nrot2,rfl,rel, jsw)
            if(nrot2(1)+nrot2(2).gt.0) then
...
c         Modify for rotated dof's : PROBABLY DOES NOT WORK
          ...
          if(nrot1(1)+nrot1(2).gt.0 .and. arotflg) then
