c$Id: jtrans.f,v 1.1 2006/11/20 20:32:37 rlt Exp $
      subroutine jtrans(lam,rlam,ebig,alp1,x1,x2h,x2,
...
c-----[--.----+----.----+----.-----------------------------------------]
c      Purpose: Form TRANSLATOR joint arrays for connecting rigid bodies.

c      Inputs:
c         lam(5)    - Value of lagrange multiplier force on joint
c         rlam(3,*) - Rotation array for rigid bodies
c         ebig(3,3) - Orientation of translator axis
...
c         r1h(3)    - Distance from body 1 mass center to x1 at t_n+alpha
c         r1(3)     - Distance from body 1 mass center to x1 at t_n+1
c         r2h(3)    - Distance from body 2 mass center to x2 at t_n+alpha
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      real*8    alp1, alpn, lam(5), ebig(3,3), rlam(9,6,*)
...
c     Interpolate the base vectors to t_n+a
      ...
      do i = 1,3
        ...
        elseif(i.eq.3) then
        call jtrots(ea1(1,1),eb1(1,2),eah(1,1),ebh(1,2),lam(i),p,s)
        end if
        do j = 1,3
          do k = 1,3
