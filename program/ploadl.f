c$Id: ploadl.f,v 1.1 2006/11/20 20:32:37 rlt Exp $
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      logical   dfl,aufl
      integer   ndf,ndm, i,j, ii,jj, nn,nrec,nrot(2),ns1, ns,nv
      ...
      if(numsl.gt.0) then
        ...
        do nn = 1,numsl
          ...
          do j = 1,nrec
            ...
            if(anglefl) then
              call pangl(mr(np(37)+nei),nel,hr(np(46)),hr(np(45)),
     &                   nrot(1))
            else
              nrot(1) = 0
            endif
            if(eulerfl) then
              call peule(mr(np(37)+nei),nel,hr(np(243)),hr(np(242)),
     &                   nrot(2))
            else
              nrot(2) = 0
            endif
            ...
            call pzero(valu,mct)
            call colred(hr(np(241)+ner),-prop,mct, valu)
...
c           Form element contributions
            ...
            call elmlib(valu,ul,xl,mr(np(37)+nei),valu,s,p,
     &                  ndf,ndm,ns1,iel,7)
