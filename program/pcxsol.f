c$Id: pcxsol.f,v 1.1 2006/11/20 20:32:37 rlt Exp $
...
c         Form: Stiffness (isw=3); Damping (isw=9); Mass (isw=5)
          ...
          call formfe(np(40),np(26),np(npart   ),nal,
     &               .true.,.false.,.false.,.false., 3,1,numel,1)
          call formfe(np(40),np(26),np(npart+ 8),nal,
     &               .true.,.false.,.false.,.false., 5,1,numel,1)
          call formfe(np(40),np(26),np(npart+16),nal,
...
c       Assemble complex tangent and modify rhs for current frequency
        ...
        call pcxasbl(omega,eta,mr(np(31)),hr(np(26)),hr(np(27)+nneq),
