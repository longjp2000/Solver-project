c$Id: perspe.f,v 1.1 2006/11/20 20:33:12 rlt Exp $
...
c      Inputs:
c         flag      - Flag, if true and not default user to provide
c                           data
...
c-----[--.----+----.----+----.-----------------------------------------]
 ...
 1    if(flag) then
        if(defalt) then
          ...
          if(ior.lt.0) then
            call pprint(' Use old parameters? (0 = new or 1 = old) :')
          endif
          errck = pinput(e,1)
          iused = e(1)
        endif
...
c     Formats
 ...
 3000 format(' *ERROR* PERSPE: Improper view specified')
