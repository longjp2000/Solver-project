c$Id: bplot.f,v 1.1 2006/11/20 20:33:12 rlt Exp $
      subroutine bplot(ct)
...
c       Divide by weights
        ...
        nfacold = nfac(1)
        setvar = palloc(112,'TEMP2',  bnumnp+1,1)
        setvar = palloc(113,'TEMP3',8*bnumel  ,1)
        setvar = palloc(114,'TEMP4',  bnumel  ,1)
...
c       Do hidden surface removal and edge definition
        ...
        if(kpers.gt.0) then
          ipb = ct(3)
          call perspz(hr(np(200)),mr(np(199)),
     &                mr(np(112)),mr(np(114)),mr(np(113)),
