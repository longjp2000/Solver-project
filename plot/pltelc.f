c$Id: pltelc.f,v 1.1 2006/11/20 20:33:12 rlt Exp $
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      integer   i,j,n,ma,maold,nc,nnc,nerr,nume
...
c             Quadratic quad
              ...
              elseif((nel.eq.8 .and. iu.ne.16) .or. nel.eq.9) then
                if(nel.eq.8) then
                  do i = 1,3
...
c             Draw border around element
              ...
              if(.not.cont .and. ipb.eq.0 ) then
                call plotl(xl(1,iplt(1)),xl(2,iplt(1)),xl(3,iplt(1)),3)
                do i = 2,iu-1
                  j = iplt(i)
                  call plotl(xl(1,j),xl(2,j),xl(3,j),2)
                end do ! i
                call plotl(xl(1,iplt(1)),xl(2,iplt(1)),xl(3,iplt(1)),2)
              else if(edgfl .and. iu.eq.5) then
