...
c  DLARUV returns a vector of n random real numbers from a uniform (0,1)
c  distribution (n <= 128).

c  This is an auxiliary routine called by DLARNV and ZLARNV.
...
c  N       (input) INTEGER
c          The number of random numbers to be generated. N <= 128.
...
c          Convert 48-bit integer to a real number in the interval (0,1)
           ...
           X( I ) = R*( DBLE( IT1 ) + R*( DBLE( IT2 ) + R*( DBLE( IT3 )
     $            + R*  DBLE( IT4 ) ) ) )
        END DO ! I
