...
c     %--------------------------------------%
     ...
     &                'dgetv0: re-orthonalization ; rnorm0 is')
          call dvout (logfil, 1, rnorm, ndigit,
     &                'dgetv0: re-orthonalization ; rnorm is')
      end if
...
c        %------------------------------------%
   ...
   50 continue
      ...
      if (msglvl .gt. 0) then
         call dvout (logfil, 1, rnorm, ndigit,
     &        'dgetv0: B-norm of initial / restarted starting vector')
      end if
      if (msglvl .gt. 3) then
         call dvout (logfil, n, resid, ndigit,
     &        'dgetv0: initial / restarted starting vector')
      end if
      ido = 99
