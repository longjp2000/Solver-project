...
c     |     by dseigt before exiting dsaup2.   |
c     | ibd points to the Ritz estimates       |
c     |     computed by dseigt before exiting  |
c     |     dsaup2.                            |
c     %----------------------------------------%
...
c     %---------------------------------------%
     ...
     &   'dseupd: Ritz values passed in from DSAUPD.')
         call dvout (logfil, ncv, workl(ibd), ndigit,
     &   'dseupd: Ritz estimates passed in from DSAUPD.')
      end if
...
c        %-------------------------------------%
     ...
     &                workl)
         ...
         if (msglvl .gt. 2) then
            call dvout (logfil, ncv, workl(irz), ndigit,
     &      'dseupd: Ritz values after calling DSGETS.')
            call dvout (logfil, ncv, workl(bounds), ndigit,
     &      'dseupd: Ritz value indices after calling DSGETS.')
         end if
...
c        | the number (nconv) reported by dsaupd.  If these two      |
c        | are different then there has probably been an error       |
c        | caused by incorrect passing of the dsaupd data.           |
c        %-----------------------------------------------------------%

         if (msglvl .gt. 2) then
             call ivout(logfil, 1, numcnv, ndigit,
     &            'dseupd: Number of specified eigenvalues')
             call ivout(logfil, 1, nconv, ndigit,
     &            'dseupd: Number of "converged" eigenvalues')
         end if
...
c        %-----------------------------------------------------------%
     ...
     &          'dseupd: NCV Ritz values of the final H matrix')
            call dvout (logfil, ncv, workl(iw), ndigit,
     &           'dseupd: last row of the eigenvector matrix for H')
         end if
...
c              %----------------------------------------------%
 ...
 30      end if
         ...
         if (msglvl .gt. 2) then
             call dvout  (logfil, ncv, workl(ihd), ndigit,
     &       'dseupd: The eigenvalues of H--reordered')
         end if
...
c        %-------------------------------------------------%
     ...
     &          'dseupd: Untransformed converged Ritz values')
         call dvout (logfil, nconv, workl(ihb), ndigit,
     &     'dseupd: Ritz estimates of the untransformed Ritz values')
      else if (msglvl .gt. 1) then
         call dvout (logfil, nconv, d, ndigit,
     &          'dseupd: Converged Ritz values')
         call dvout (logfil, nconv, workl(ihb), ndigit,
     &     'dseupd: Associated Ritz estimates')
      end if
