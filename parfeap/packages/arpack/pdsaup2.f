c$Id: pdsaup2.f,v 1.1 2006/11/20 20:43:46 rlt Exp $
...
c  IDO, BMAT, N, WHICH, NEV, TOL, RESID: same as defined in pdsaupd.
...
c     %---------------------------------------------%
   ...
   10 continue
      ...
      if (getv0) then
         call pdgetv0 (ido, bmat, 1, initv, n, 1, v, ldv, resid, rnorm,
     &                 ipntr, workd, info)
...
c     %----------------------------------------------------------%
      ...
      call pdsaitr (ido, bmat, n, 0, nev0, mode, resid, rnorm, v, ldv,
...
c     %--------------------------------------------------------------%
 ...
 1000 continue
     ...
     &           'pdsaup2: **** Start of major iteration number ****')
         end if
         if (msglvl .gt. 1) then
            call ivout (logfil, 1, nev, ndigit,
     &       'pdsaup2: The length of the current Lanczos factorization')
            call ivout (logfil, 1, np, ndigit,
     &           'pdsaup2: Extend the Lanczos factorization by')
         end if
...
c        %------------------------------------------------------------%
   ...
   20    continue
         ...
         call pdsaitr (ido, bmat, n, nev, np, mode, resid, rnorm, v,
...
c           %-----------------------------------------------------%
     ...
     &         'pdsaup2: Current B-norm of residual for factorization')
         end if
...
c        %-------------------%
     ...
     &                  'pdsaup2: NEV, NP, NCONV are')
            call dvout (logfil, kplusp, ritz, ndigit,
     &           'pdsaup2: The eigenvalues of H')
            call dvout (logfil, kplusp, bounds, ndigit,
     &        'pdsaup2: Ritz estimates of the current NCV Ritz values')
         end if
...
c           %------------------------------------------%
            ...
            if (msglvl .gt. 1) then
               call dvout (logfil, kplusp, ritz, ndigit,
     &            'pdsaup2: Sorted Ritz values.')
               call dvout (logfil, kplusp, bounds, ndigit,
     &            'pdsaup2: Sorted ritz estimates.')
            end if
...
c           %---------------------------------------%
     ...
     &         'pdsaup2: no. of "converged" Ritz values at this iter.')
            if (msglvl .gt. 1) then
               kp(1) = nev
               kp(2) = np
               call ivout (logfil, 2, kp, ndigit,
     &              'pdsaup2: NEV and NP are')
               call dvout (logfil, nev, ritz(np+1), ndigit,
     &              'pdsaup2: "wanted" Ritz values.')
               call dvout (logfil, nev, bounds(np+1), ndigit,
     &              'pdsaup2: Ritz estimates of the "wanted" values ')
            end if
         end if
...
c        %---------------------------------------------------------%
         ...
         if (msglvl .gt. 2) then
            call ivout (logfil, 1, np, ndigit,
     &                  'pdsaup2: The number of shifts to apply ')
            call dvout (logfil, np, workl, ndigit,
     &                  'pdsaup2: shifts selected')
            if (ishift .eq. 1) then
               call dvout (logfil, np, bounds, ndigit,
     &                  'pdsaup2: corresponding Ritz estimates')
             end if
         end if
...
c           call second (t3)
  ...
  130    continue
         ...
         if (msglvl .gt. 2) then
            call dvout (logfil, 1, rnorm, ndigit,
     &           'pdsaup2: B-norm of residual for NEV factorization')
            call dvout (logfil, nev, h(1,2), ndigit,
     &           'pdsaup2: main diagonal of compressed H matrix')
            call dvout (logfil, nev-1, h(2,1), ndigit,
     &           'pdsaup2: subdiagonal of compressed H matrix')
         end if
