c$Id: cnts2d.f,v 1.1 2006/11/20 20:32:46 rlt Exp $
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      integer   istgt,istfr,n1,n2,i,j,resfl
...
c               Form stiffness and residual from material law
                ...
                call cstif1 (cp0,ch2(1,kset),
     &                       ch3(1,kset),8,tanm,resv)
...
c             Compute geometrical parameters
     ...
     &                      cp0,ch1(1,kset),ch2(1,kset),ch3(1,kset))
...
c               Form stiffness and residual
                ...
                call penares (resfl,cp0,ch2(1,kset),
     &                        ch3(1,kset),resv)
...
c       Set flag for geometry check
        ...
        dopt  = cp0(2,3) ! Detection method
        lcnt  = lcnt + 1
        liter = max(2.d0,cp0(3,3))
...
c     Called from PPLOTF for plot of contact geometry
      ...
      elseif (csw.eq.305) then
        if(nint(cs02(1,0)).eq.6) then
          call c2rigplt (ix1,cs02(2,0),2,6)
        else
          call c2geoplt (ix1,ix2,2,6)
        endif
