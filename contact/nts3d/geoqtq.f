c$Id: geoqtq.f,v 1.1 2006/11/20 20:32:46 rlt Exp $
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      logical    flag2,flag,ueberall, conseg(*),cpoint(*)
...
c           if no contact to segment found before, remember xi and segment
...
110     continue
      enddo     !  Loop over elements of patch of closest node
...
c           cpoint(i) = .true.
        ...
        if (zahl.ge.anzseg) then
          ueberall = .true.
          do i = 1,anzseg
            if(.not.cpoint(i)) ueberall = .false.
          enddo
