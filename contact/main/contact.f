c$Id: contact.f,v 1.1 2006/11/20 20:32:46 rlt Exp $
...
c     100 <= CSW <= 199         --> Direct call to do preliminary activity

c     200 <= CSW                --> Direct call to do equivalent activity
...
c     300   x                       Init contact flags for each new problem
...
c     308   x                       Set profile and range to plot variable
c     309   x                       Switch contact on/off during execution.
c     310   x                       Reset penalty during execution.
c     311   x                       Compute average stresses and strains
c     312   x                       Adjust contact elements for tied nodes
c     313   x                       Activate history variables
c     314   x                       Update lagrange multipliers
c     315   x                       Output contact surface facet list


c     400 <= CSW                --> Special internal call

c     400   1                       Perform "once" activity for the problem
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      integer   csw, lcc0,kp,npair,kd,ncel, typ
      real*8    td(15)
...
c     Skip if contact is off
      ...
      if ((csw.ne.  1) .and. (csw.ne.309) .and.
     &    (csw.ne.300) .and. (csw.ne.400)) then
        if (.not.ifct) return
      endif
...
c         Allocate memory for material data (max c_lmv=50 input constants)
...
c         REMARK - element storage will be extended after reading elements
...
c       Assemble single array with all active contact element connections
...
c       Optimize profile and reset equation numbers
        ...
        if(optflg .or. lagrm) then
          ...
          npair = ndf*numnp
          do kp = 0,npair-1
            mr(np(31)+kp) = mr(np(31)+kp+npair)
          end do

          call seteq(mr(np(31)),mr(np(99)),mr(np(182)),mr(np(100)),
     &               mr(np(101)),.false.)
...
c         Adjust equation numbers when elements have no multiplers
          ...
          if(np(211).eq.0) then
            call newnum (mr(np(224)),mr(np(31)),mr(np(89)),
     &                   mr(np(89)+numnp),mr(np(99)),mr(np(101)))

c         Adjust equation numbers when elements have multiplers

          else
            call newnuml(mr(np(224)),mr(np(31)),mr(np(211)),mr(np(212)),
...
c           Compute compressed profile array
            ...
            call elcnt(numel,nen,nen1,mr(np(31)),mr(np(33)),
     &                 mr(np(111)), 1)

c           Consider contact elements connected to each node

            if(numcels.gt.0) then
              call elcnt(numcels,ncen,ncen1,mr(np(31)),
...
c           Set list of elements connected to nodes
     ...
     &                  mr(np(111)),mr(np(112)),kp,1)
            if(numcels.gt.0) then
              call pelcon(numcels,ncen,ncen1,mr(np(168)),mr(np(31)),
     &                    mr(np(111)),mr(np(112)),kp, -1)
            endif
...
c         Reset profile of continuum discretization
          ...
          call rstprf(mr(np(20+npart)),mr(np(34)),mr(np(31)),mr(np(33)),
...
c     Called from OUTARY for dump of contact arrays          csw = 303
      ...
      elseif (csw.eq.303) then
        call cdebug (0,yyy(16:19),'interactive')
...
c     WARNING: Contact can be switched on only if set at start of problem.
...
c         Pair loop
          ...
          do kp = 1,numcp
            npair = kp
            call cptest(hr(ccp(3)),npair,surfl)
          end do ! kp

          if(surfl) then
            setvar = palloc( 192,'SURFP', numcs ,  1)
            call flaechen(hr(ccp(1)),mr(np(133)),mr(np(192)))
          endif
