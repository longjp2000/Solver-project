c$Id: adjleqn.f,v 1.1 2006/11/20 20:32:46 rlt Exp $
...
c-----[--+---------+---------+---------+---------+---------+---------+-]
c      Purpose: Compute 'offset' for each Lagrange multiplier associated
c               to a 'maximum' node number (some multipliers could share
c               same maximum node number.

c      Inputs:
c        iad(n,*) - 2: stores maximum node associated with multiplier
c                      node 'n'; 3: stores number of multiplers for node.
c        numnp    - Number of nodes in mesh.

c      Output:
c        iad(n,*) - 1: has total number of multiplier equation for node
c                   iad(n,2).
