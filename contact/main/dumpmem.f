c$Id: dumpmem.f,v 1.1 2006/11/20 20:32:46 rlt Exp $
...
c       Perform dump
     ...
     &                     (cfea(kc),kc=1,nfe(3)),(cusr,kc=1,nuc(3))
          ...
          do kr = 1,nr0
            if (ifdb) then
              write (debf,2004) (cp0(kr,kc,kp),kc = n0c(3),nc0(3))
            endif
            if (screen) then
              write (iow,2004) (cp0(kr,kc,kp),kc = n0c(3),nc0(3))
            endif
            if ((ior.lt.0) .and. (screen)) then
              write (*,2004) (cp0(kr,kc,kp),kc = n0c(3),nc0(3))
            endif
          end do ! kr
          if ((ior.lt.0) .and. (screen) .and.
     &        (.not.pcomp(c,'r',1))          ) then
...
c         Perform minimal printout
          ...
          if (ifdb) then
            write (debf,2005) rnsurf,ks,neps,nope
          endif
          if (screen) then
            write (iow,2005) rnsurf,ks,neps,nope
          endif
          if ((ior.lt.0) .and. (screen)) then
            write (*,2005) rnsurf,ks,neps,nope
          endif
...
c         Perform dump of nodes if requested
          ...
          if ((ks.ge.nf) .and. (ks.le.nl)) then
            do ke = 1,neps
              i0 = ofs + (ke-1)*dnope - 1
              if (ifdb) then
                write (debf,2006) ke,(ics(i0+kn),kn = 1,nope)
              endif
              if (screen) then
                write (iow,2006) ke,(ics(i0+kn),kn = 1,nope)
              endif
              if ((ior.lt.0) .and. (screen)) then
                write (*,2006) ke,(ics(i0+kn),kn = 1,nope)
              endif
            end do ! ke
          endif
          if ((ior.lt.0) .and. (screen) .and.
...
2017  format ( a,i10)
2018  format ( 1x,20a10)
2019  format ( 1x,20i10)
