c$Id: continit.f,v 1.1 2006/11/20 20:32:46 rlt Exp $
      subroutine continit ()
...
c     Command # 1 - SURF
      ...
      ncom = 1
      nty(ncom)  = 6   ! # of surface TYPES defined
      nfe(ncom)  = 0
      nop(ncom)  = 0
      nsc(ncom)  = 5   ! # of defined SUB-COMMANDS
      nso(ncom)  = 5   ! max # of sub-command OPTIONS for each sub-command
...
c     Command # 3 - PAIR
      ...
      nfe(ncom)  =  7   ! # of defined FEATURES
      nop(ncom)  =  6   ! max # of OPTIONS for each feature
      nsc(ncom)  =  0
      nso(ncom)  =  0
...
c     TYPES definition
      ...
      ncom = 1
      cis(typ(ncom,1))   = 'line'  ! LINE 2d element with 2 or more nodes
      cis(typ(ncom,2))   = 'tria'  ! TRIAngular 3d element with 3 or more nodes
      cis(typ(ncom,3))   = 'quad'  ! QUADrilateral 3d element with 8/more node
      cis(typ(ncom,4))   = 'beam'  ! BEAM 3d element with 2 or more nodes
      cis(typ(ncom,5))   = 'poin'  ! POINt element with 1 node.
      cis(typ(ncom,6))   = 'rigi'  ! RIGId element surface function

c     FEATURES definition
c     This definition is used to allocate an equivalent amount of columns
...
c     SUB-COMMANDS definitions
c     This definition is used to switch the data reading to the corresponding
...
c     cis(sop(ncom,1,1))   = '    '
      ...
      cis(sop(ncom,3,3))   = 'exte'  ! BLOCk    EXTErnal   option
...
c     10 subroutines are actually available
      ...
      ncom = 2
      cis(typ(ncom,1))   = 'stan'  ! STANdard "rigid" material: Coulomb friction
      cis(typ(ncom,2))   = 'nlfr'  ! "rigid" material: NonLinear FRiction
      cis(typ(ncom,3))   = 'user'  ! User defined material: type data in cm0
...
c     Command # 3 - PAIR
c     TYPES definition
c     This definition is used to switch the solution to the corresponding
...
c     10 contact elements are actually available
      ...
      cis(typ(ncom,26))   = 'tied'  ! Tied surface to surface interface
...
c     OPTIONS definitions
      ...
      cis(opp(ncom,1,1))   = 'off'   ! SWIT - Switch off part or total pair
      cis(opp(ncom,1,2))   = 'on'    ! SWIT - Switch on  part or total pair
      cis(opp(ncom,1,3))   = 'timf'  ! SWIT - Switch on/off with time function

      cis(opp(ncom,2,1))   = 'pena'  ! SOLM - PENAlty solution method
      cis(opp(ncom,2,2))   = 'lagm'  ! SOLM - LAGrangian Multipliers method
      ...
      cis(opp(ncom,5,3))   = 'hset'  ! AUGM - History SET based augmentation
      cis(opp(ncom,5,4))   = 'lise'  ! AUGM - LImited Stiffness Enhanced mode
